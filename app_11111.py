...
async def get_admin_dashboard(
    ...
    try:
        ...
        safety_metrics = {
            ...
            "blocked_content": db.query(SafetyLog).filter(SafetyLog.content_blocked == True).count(),
            "allowed_content": db.query(SafetyLog).filter(SafetyLog.content_blocked == False).count(),
            "recent_blocks": db.query(SafetyLog).filter(
                SafetyLog.content_blocked == True,
                SafetyLog.timestamp >= seven_days_ago
            ).count(),
            "user_input_blocks": db.query(SafetyLog).filter(
                SafetyLog.event_type == 'user_input',
                SafetyLog.content_blocked == True
            ).count(),
            "ai_response_blocks": db.query(SafetyLog).filter(
                SafetyLog.event_type == 'ai_response',
                SafetyLog.content_blocked == True
            ).count()
        }
...
async def get_admin_safety_logs(
    ...
    content_blocked: Optional[bool] = None,
    admin_user: dict = Depends(get_current_admin_user),
    db: Session = Depends(get_chatbot_db)
):
    ...
    try:
        ...
        # Apply filters
        if event_type:
            query = query.filter(SafetyLog.event_type == event_type)
        if content_blocked is not None:
            query = query.filter(SafetyLog.content_blocked == content_blocked)
        ...
        blocked_count = db.query(SafetyLog).filter(SafetyLog.content_blocked == True).count()
        allowed_count = db.query(SafetyLog).filter(SafetyLog.content_blocked == False).count()
            ...
            "safety_logs": [
                {
                    ...
                    "original_text": log.original_text[:200] + "..." if log.original_text and len(log.original_text) > 200 else log.original_text,
                    "processed_text": log.processed_text[:200] + "..." if log.processed_text and len(log.processed_text) > 200 else log.processed_text,
                    "content_safety_result": log.content_safety_result,
                    "content_blocked": log.content_blocked,
                    "blocked_categories": log.blocked_categories,
                    "pii_detection_result": log.pii_detection_result,
                    "pii_detected": log.pii_detected,
                    "pii_entities": log.pii_entities,
            ...
            if safety_result.get('blocked', False):
                ...
                try:
                    ...
                    safety_log = SafetyLog(
                        ...
                        blocked_categories=safety_result.get('blocked_categories', []),
                        pii_detection_result=safety_result.get('pii_result', {}),
                        pii_detected=safety_result.get('pii_detected', False),
                        pii_entities=safety_result.get('pii_result', {}).get('entities', []),
                        timestamp=datetime.utcnow()
                    )
                ...
                try:
                    ...
                    safety_log = SafetyLog(
                        ...
                        event_type='user_input',
                        original_text=request.message,
                        processed_text=safety_result.get('processed_content', request.message),
                        content_safety_result=safety_result,
                ...
                if response_safety_result.get('blocked', False):
                    ...
                    try:
                        ...
                        safety_log = SafetyLog(
                            ...
                            content_safety_result=response_safety_result,
                            content_blocked=True,
                            blocked_categories=response_safety_result.get('blocked_categories', []),
                            pii_detection_result=response_safety_result.get('pii_result', {}),
                            pii_detected=response_safety_result.get('pii_detected', False),
                            pii_entities=response_safety_result.get('pii_result', {}).get('entities', []),
                            timestamp=datetime.utcnow()
                        )
                        db_session.add(safety_log)
                        db_session.commit()
                    ...
                    try:
                        ...
                        safety_log = SafetyLog(
                            ...
                            event_type='ai_response',
                            original_text=response_text,
                            processed_text=response_safety_result.get('processed_content', response_text),
                            content_safety_result=response_safety_result,
                            content_blocked=False,
                            blocked_categories=[],
                            pii_detection_result=response_safety_result.get('pii_result', {}),
