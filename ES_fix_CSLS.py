...
@st.cache_data(ttl=60)
def get_admin_safety_logs_cached(headers, event_type=None, content_blocked=None):
    """Get admin safety logs with caching"""
    try:
        session = get_requests_session()
        params = {}
        if event_type:
        ...
        response = session.get(f"{API_BASE_URL}/admin/safety-logs", headers=headers, params=params, timeout=15)
        if response.status_code == 200:
            return True, response.json()
        else:
            return False, f"Status: {response.status_code}"
    ...
    if st.session_state['authenticated'] == False:
        ...
        def render_admin_panel():
            ...
            admin_tab1, admin_tab2, admin_tab3, admin_tab4, admin_tab5 = st.tabs([
                "📊 Dashboard", "📄 Documents", "👥 Users", "📈 Analytics", "🛡️ Safety"
            ])
            ...
            with admin_tab1:
                ...
                if success:
                    ...
                    # Safety metrics
                    safety_metrics = dashboard_data.get("safety_metrics", {})
                    if safety_metrics:
                        st.subheader("Content Safety Overview (User Input Only)")
                        col1, col2, col3, col4 = st.columns(4)
                        
                        with col1:
                            st.metric("Total User Input Checks", safety_metrics.get("total_safety_checks", 0))
                        with col2:
                            st.metric("Blocked User Input", safety_metrics.get("blocked_content", 0))
                        with col3:
                            st.metric("Recent Blocks (7d)", safety_metrics.get("recent_blocks", 0))
                        with col4:
                            st.metric("PII Detected", safety_metrics.get("pii_detected_count", 0))
                        
                        # Additional row for rates
                        col1, col2, col3, col4 = st.columns(4)
                        with col1:
                            blocked = safety_metrics.get("blocked_content", 0)
                            total = safety_metrics.get("total_safety_checks", 1)
                            block_rate = (blocked / total * 100) if total > 0 else 0
                            st.metric("Block Rate", f"{block_rate:.1f}%")
                        with col2:
                            pii_count = safety_metrics.get("pii_detected_count", 0)
                            pii_rate = (pii_count / total * 100) if total > 0 else 0
                            st.metric("PII Detection Rate", f"{pii_rate:.1f}%")
            ...
            with admin_tab5:
                st.subheader("Content Safety Monitoring")
                ...
                with col1:
                    event_type_filter = st.selectbox(
                        ...
                        ["All", "user_input"],
                        key="safety_event_type_filter"
                    )
                with col2:
                    blocked_filter = st.selectbox(
                        "Filter by Status",
                        ["All", "Blocked", "Allowed"],
                        key="safety_blocked_filter"
                    )
                with col3:
                    if st.button("Refresh Safety Logs"):
                        get_admin_safety_logs_cached.clear()
                
                # Get safety logs
                event_type_param = None if event_type_filter == "All" else event_type_filter
                blocked_param = None if blocked_filter == "All" else (True if blocked_filter == "Blocked" else False)
                
                success, safety_data = get_admin_safety_logs_cached(headers, event_type_param, blocked_param)
                if success:
                    safety_logs = safety_data.get("safety_logs", [])
                    ...
                    with col2:
                        st.metric("Blocked Content", summary.get("blocked_count", 0))
                    with col3:
                        st.metric("Allowed Content", summary.get("allowed_count", 0))
                    with col4:
                        total = summary.get("total_processed", 1)
                        blocked = summary.get("blocked_count", 0)
                        block_rate = (blocked / total * 100) if total > 0 else 0
                        st.metric("Block Rate", f"{block_rate:.1f}%")
                    
                    st.write(f"**Showing {len(safety_logs)} safety logs**")
                    
                    if safety_logs:
                        # Safety logs table
                        for log in safety_logs:
                            action_color = "🔴" if log.get('content_blocked') else "🟢"
                            event_type_icon = "👤" if log.get('event_type') == 'user_input' else "🤖"
                            status_text = "Blocked" if log.get('content_blocked') else "Allowed"
                            
                            with st.expander(f"{action_color} {event_type_icon} {log.get('event_type', 'Unknown')} - {status_text} - {log.get('timestamp', 'N/A')[:19]}"):
                                col1, col2 = st.columns(2)
                                
                                with col1:
                                    st.write(f"**User ID:** {log.get('user_id', 'N/A')}")
                                    st.write(f"**Session ID:** {log.get('session_id', 'N/A')[:8]}..." if log.get('session_id') else "N/A")
                                    st.write(f"**Event Type:** {log.get('event_type', 'N/A')}")
                                    st.write(f"**Content Blocked:** {'Yes' if log.get('content_blocked') else 'No'}")
                                
                                with col2:
                                    st.write(f"**Timestamp:** {log.get('timestamp', 'N/A')}")
                                    
                                    # Content safety details
                                    content_safety_result = log.get('content_safety_result', {})
                                    if content_safety_result and isinstance(content_safety_result, dict):
                                        if content_safety_result.get('content_safety_result'):
                                            st.write("**Content Safety Issues:**")
                                            safety_details = content_safety_result['content_safety_result']
                                            if isinstance(safety_details, dict):
                                                for category, details in safety_details.items():
                                                    if isinstance(details, dict) and details.get('severity', 0) > 0:
                                                        st.write(f"  • {category.title()}: Severity {details['severity']}")
                                        
                                        # Show if content was blocked and why
                                        if content_safety_result.get('blocked'):
                                            blocked_categories = content_safety_result.get('blocked_categories', [])
                                            if blocked_categories:
                                                st.write(f"**Blocked Categories:** {', '.join(blocked_categories)}")
                                    
                                    # Blocked categories
                                    if log.get('blocked_categories'):
                                        st.write(f"**Blocked Categories:** {', '.join(log['blocked_categories'])}")
                                    
                                    # PII detection details
                                    if log.get('pii_detected'):
                                        pii_entities = log.get('pii_entities', [])
                                        if isinstance(pii_entities, list):
                                            st.write(f"**PII Detected:** {len(pii_entities)} entities")
                                            if pii_entities:
                                                entity_types = []
                                                for entity in pii_entities:
                                                    if isinstance(entity, dict):
                                                        entity_types.append(entity.get('category', 'Unknown'))
                                                if entity_types:
                                                    st.write(f"  • Types: {', '.join(set(entity_types))}")
                                
                                # Content preview
                                st.write("**Original Content:**")
                                st.code(log.get('original_text', 'N/A'), language=None)
                                
                                if log.get('processed_text') and log.get('processed_text') != log.get('original_text'):
                                    st.write("**Processed Content (PII Masked):**")
                                    st.code(log.get('processed_text', 'N/A'), language=None)
                                
                                # Raw safety result (expandable)
                                if content_safety_result and isinstance(content_safety_result, (dict, list)):
                                    with st.expander("Raw Content Safety Result"):
                                        st.json(content_safety_result)
                                
                                # Raw PII result (expandable)
                                pii_result = log.get('pii_detection_result')
                                if pii_result and isinstance(pii_result, (dict, list)):
                                    with st.expander("Raw PII Detection Result"):
                                        st.json(pii_result)
                    else:
                        st.info("No safety logs found matching the filters.")
                else:
