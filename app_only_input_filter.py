...
async def get_admin_dashboard(
    ...
    try:
        ...
        safety_metrics = {
            "total_safety_checks": db.query(SafetyLog).filter(SafetyLog.event_type == 'user_input').count(),
            "blocked_content": db.query(SafetyLog).filter(
                SafetyLog.event_type == 'user_input',
                SafetyLog.content_blocked == True
            ).count(),
            "allowed_content": db.query(SafetyLog).filter(
                SafetyLog.event_type == 'user_input',
                SafetyLog.content_blocked == False
            ).count(),
            ...
            "pii_detected_count": db.query(SafetyLog).filter(
                SafetyLog.event_type == 'user_input',
                SafetyLog.pii_detected == True
            ).count()
        }
...
async def chat_with_bot(
    ...
    try:
        ...
        try:
            ...
            # Skip safety check on AI response to preserve links and formatting
            print("Skipping safety check on AI response to preserve links and formatting")
