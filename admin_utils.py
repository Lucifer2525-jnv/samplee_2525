...
def get_user_activity_stats(db: Session, days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        daily_activity = db.query(
            ...
            ChatHistory.timestamp >= cutoff_date
        ).group_by(func.date(ChatHistory.timestamp)).order_by('date').all()
...
def get_feedback_analytics(db: Session, days: int = 30) -> Dict[str, Any]:
    ...
    try:
        ...
        quality_metrics = db.query(
            func.avg(func.cast(Feedback.is_accurate, Integer)).label('accuracy_rate'),
            func.avg(func.cast(Feedback.is_relevant, Integer)).label('relevance_rate'),
            func.avg(func.cast(Feedback.is_clear, Integer)).label('clarity_rate'),
            func.avg(func.cast(Feedback.is_complete, Integer)).label('completeness_rate')
...
def get_system_metrics(db: Session) -> Dict[str, Any]:
    ...
    try:
        ...
        ).group_by(ChatHistory.session_id).scalar()
