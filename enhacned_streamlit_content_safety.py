...
@st.cache_data(ttl=60)
def get_admin_safety_logs_cached(headers, content_type=None, action_taken=None):
            ...
            with admin_tab5:
                ...
                with col1:
                    content_type_filter = st.selectbox(
                        ...
                        key="safety_content_type_filter"
                    )
                with col2:
                    action_filter = st.selectbox(
                        "Filter by Action",
                        ["All", "blocked", "allowed"],
                        key="safety_action_filter"
                    )
                with col3:
                ...
                success, safety_data = get_admin_safety_logs_cached(headers, content_type_param, action_param)
                if success:
                    ...
                    if safety_logs:
                        ...
                        for log in safety_logs:
                            action_color = "🔴" if log.get('action_taken') == 'blocked' else "🟢"
                            content_type_icon = "👤" if log.get('content_type') == 'user_input' else "🤖"
                            
                            with st.expander(f"{action_color} {content_type_icon} {log.get('content_type', 'Unknown')} - {log.get('action_taken', 'Unknown')} - {log.get('timestamp', 'N/A')[:19]}"):
                                col1, col2 = st.columns(2)
                                
                                with col1:
                                    st.write(f"**User ID:** {log.get('user_id', 'N/A')}")
                                    st.write(f"**Session ID:** {log.get('session_id', 'N/A')[:8]}..." if log.get('session_id') else "N/A")
                                    st.write(f"**Content Type:** {log.get('content_type', 'N/A')}")
                                    st.write(f"**Action Taken:** {log.get('action_taken', 'N/A')}")
                                
                                with col2:
                                    st.write(f"**Timestamp:** {log.get('timestamp', 'N/A')}")
                                    
                                    # Safety result details
                                    safety_result = log.get('safety_result', {})
                                    if safety_result:
                                        if safety_result.get('content_safety_result'):
                                            st.write("**Content Safety Issues:**")
                                            for category, details in safety_result['content_safety_result'].items():
                                                if details.get('severity', 0) > 0:
                                                    st.write(f"  • {category.title()}: Severity {details['severity']}")
                                        
                                        if safety_result.get('pii_result', {}).get('entities'):
                                            pii_count = len(safety_result['pii_result']['entities'])
                                            st.write(f"**PII Detected:** {pii_count} entities")
                                
                                # Content preview
                                st.write("**Original Content:**")
                                st.code(log.get('original_content', 'N/A'), language=None)
                                
                                if log.get('processed_content') and log.get('processed_content') != log.get('original_content'):
                                    st.write("**Processed Content (PII Masked):**")
                                    st.code(log.get('processed_content', 'N/A'), language=None)
                                
                                # Raw safety result (expandable)
                                if safety_result:
                                    with st.expander("Raw Safety Result"):
